#!/usr/bin/env python3
"""Reproduce the direct zero-frequency epoxy viscosity script supplied separately.

This is a legacy/reference calculation, not the recommended generic workflow.
It mirrors the supplied script's algebra,

    eta = (1/V) sum_p C_gamma Gamma_p nu / (tu^2 lambda_p)^2,

including omission of the final mode.  The supplied script used a symmetric
|lambda| < 1.0 cutoff, whereas G_dp.f90 uses |lambda| < 2.57.  This program
makes that cutoff explicit so the sensitivity can be quantified.
"""
...
def main() -> None:
    ...
    print(f"eta = {eta_pa_s/1.0e9:.12g} GPa s")
