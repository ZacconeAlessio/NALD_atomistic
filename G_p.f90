...
f=-2.906*(1e+6)*((tu**2)*swp(i)-w*w)*gamma(i)*(((tu**2)*swp(i)-w*w)**(+2)+1*nu*nu*w*w)**(-1)
sum=sum+f
end do
write(13,*)w*(tu**(-1)), (1*(GA)+sum*constant)*(1e-9)
end do
write(*,15) (101325*(Gx(1)+Gy(1)+Gz(1))/3)
end do
